...
interface StructureInfo {
  ...
  occurrences: number;
  examples: string[];
  type: "string" | "date" | "number" | "boolean" | "array" | "duration" | "unknown";
}
...
function updateStructures(index: TaskIndex, fields: Record<string, SemanticField> | null | undefined) {
  ...
  for (const [key, field] of Object.entries(fields)) {
    ...
    if (typeof value === "string") {
      if (/^\d{4}-\d{2}-\d{2}/.test(value) || /today|tomorrow|monday|tuesday|wednesday|thursday|friday/i.test(value)) {
        struct.type = "date";
...
function displayTask(task: Task, options: { verbose?: boolean; showRelations?: boolean } = {}) {
  ...
  if (deadline && !task.completed) {
    if (isOverdue(deadline)) {
      ...
      deadlineIndicator = " \x1b[33m📅 TODAY\x1b[0m";
    }
  }
  ...
  for (const key of keyFields) {
    if (task.fields[key]) {
      ...
      shownFields.push(`${key}: ${color}${formatFieldValue(task.fields[key].value)}\x1b[0m`);
    }
  }
